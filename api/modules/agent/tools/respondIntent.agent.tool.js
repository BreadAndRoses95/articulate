        ...
        const recognizedEntitiesNames = _.map(recognizedEntities, (recognizedEntity) => {
            ...
            //If the slot wasn't part of the scenario slots array. This means that the slot is a system entity
                ...
                if (recognizedEntity.entity.indexOf('sys.spacy_') !== -1 || recognizedEntity.entity.indexOf('sys.duckling_') !== -1 || recognizedEntity.entity.indexOf('sys.regex_') !== -1) {
                    if (recognizedEntity.entity.indexOf('sys.regex_') !== -1) {
                        const entityRegexName = recognizedEntity.entity.split('sys.regex_')[1];
                        if (intentSlotEntitiesNames.indexOf(entityRegexName) > -1 ) {
                            const slotNameToFill = intentSlotNames[intentSlotEntitiesNames.indexOf(entityRegexName)];
                            conversationStateObject.context[conversationStateObject.context.length - 1].slots[slotNameToFill] = GetEntityValue(recognizedEntity, conversationStateObject.text);
                            conversationStateObject.currentContext.slots[slotNameToFill] = GetEntityValue(recognizedEntity, conversationStateObject.text);
                            return entityRegexName;
                        }
                    }
