...
const trainSingleDomain = (server, rasa, agent, request, callback) => {
    let model = Guid.create().toString();
    Async.waterfall(
      [
          (cb) => {
          BuildSingleDomainTrainingData(
          server,
          agent.id,
          ...
          server.inject(formatRequest(`/agent/${agent.id}/settings/${trainingSet.numberOfIntents === 1 ? "entity" : "intent"}ClassifierPipeline`),
          (res) => {
              if (res.statusCode !== 200) {
                const error = Boom.create(
                res.statusCode,
                "An error occurred getting the data of the pipeline"
              );
                return cb(error, null);
            }
              const stringTrainingSet = JSON.stringify(
          ...
          Wreck.post(
          `${rasa}/train?project=${agent.agentName}&model=${modelFolderName}`,
            {
                payload: rasaPayload,
                headers: {
          ...
          (err) => {
              if (err) {
                ...
                "An error occurred calling the training process."
              );
                return cb(error);
            }
              return cb(null);
          ...
          Wreck.delete(
          `${rasa}/models?project=${agent.agentName}&model=default_${
            agent.model
          }`,
          {},
          (err) => {
