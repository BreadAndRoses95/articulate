...
const getDomainData = (server, domainId, cb) => {
    Async.parallel(
      {
          entities: (callback) => {
          server.inject(formatRequest(`/domain/${domainId}/entity`), (res) => {
            if (res.statusCode !== 200) {
              const error = Boom.create(
              res.statusCode,
              `An error occurred getting the entities of the domain ${domainId}`
            );
          ...
          server.inject(formatRequest(`/domain/${domainId}/intent`), (res) => {
            if (res.statusCode !== 200) {
              const error = Boom.create(
              res.statusCode,
              `An error occurred getting the intents of the domain ${domainId}`
            );
              return callback(error, null);
          }
